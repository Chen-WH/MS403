...
double k_v = 0.1, k_w = 0.5, k_pz = 0.3;  // 比例系数
double epsilon = 1e-3;
double w = 0.2;
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher velocity_pub0 = nh0.advertise<geometry_msgs::Twist>("/uav0/mavros/setpoint_velocity/cmd_vel_unstamped", 10);
    ...
    ros::Publisher velocity_pub1 = nh1.advertise<geometry_msgs::Twist>("/uav1/mavros/setpoint_velocity/cmd_vel_unstamped", 10);
    ...
    ros::Publisher velocity_pub2 = nh2.advertise<geometry_msgs::Twist>("/uav2/mavros/setpoint_velocity/cmd_vel_unstamped", 10);
    ...
    ros::Publisher velocity_pub3 = nh3.advertise<geometry_msgs::Twist>("/uav3/mavros/setpoint_velocity/cmd_vel_unstamped", 10);
...
///>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    while (ros::ok()) {
        ...
        update_pub();
        velocity_pub0.publish(vel0);
        velocity_pub1.publish(vel1);
        velocity_pub2.publish(vel2);
        velocity_pub3.publish(vel3);
        ros::spinOnce();
        rate.sleep();
    }
...
void update_pub(){
    vel0.angular.z = k_w*u_phi(0);
    vel1.angular.z = k_w*u_phi(1);
    vel2.angular.z = k_w*u_phi(2);
    vel3.angular.z = k_w*u_phi(3);
    vel0.linear.z = k_pz*(z_lock - z(0));
    vel1.linear.z = k_pz*(z_lock - z(1));
    vel2.linear.z = k_pz*(z_lock - z(2));
    vel3.linear.z = k_pz*(z_lock - z(3));
    vel0.linear.x = k_v*(a*cos(theta(0) + k_w*u_theta(0) + w) - x(0));
    vel1.linear.x = k_v*(a*cos(theta(1) + k_w*u_theta(1) + w) - x(1));
    vel2.linear.x = k_v*(a*cos(theta(2) + k_w*u_theta(2) + w) - x(2));
    vel3.linear.x = k_v*(a*cos(theta(3) + k_w*u_theta(3) + w) - x(3));
    vel0.linear.y = k_v*(b*sin(theta(0) + k_w*u_theta(0) + w) - y(0));
    vel1.linear.y = k_v*(b*sin(theta(1) + k_w*u_theta(1) + w) - y(1));
    vel2.linear.y = k_v*(b*sin(theta(2) + k_w*u_theta(2) + w) - y(2));
    vel3.linear.y = k_v*(b*sin(theta(3) + k_w*u_theta(3) + w) - y(3));
}