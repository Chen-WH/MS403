/* 
 * Date: 2021-12-16
 * Description: Formation selection obstacle avoidance
...
/* 队形变换参数设置 */
int formationA_type = 3;            // 行进编队种类
int formationB_type = 3;            // 避障编队种类
double k_zeta = 0.2;                // 整体编队代价估计量一致性系数
double k_xi = 0.05;                 // 局部编队代价实时量系数
double characteristic_1 = 0.6;      // 行进编队特征长度（圆形）
double characteristic_2 = 0.3;      // 行进编队特征长度（星型及楔形）
double characteristic_3;            // 避障编队特征长度（单列及双列），自主根据障碍物宽度进行调整
double characteristic_4;            // 避障编队特征长度（三列），自主根据障碍物宽度进行调整
...
int main(int argc, char** argv) {
    ...
    Eigen::MatrixXd colume1(swarm_robot_id.size(), 2);       // 单路纵队相对位置 [x y]
    Eigen::MatrixXd colume2(swarm_robot_id.size(), 2);       // 两路纵队相对位置
    Eigen::MatrixXd colume3(swarm_robot_id.size(), 2);       // 三路纵队相对位置
    ...
    characteristic_3 = std::fabs(obstacle_pose[0][1] - obstacle_pose[1][1])/2 - threshold - 0.1;
    characteristic_4 = std::fabs(obstacle_pose[0][1] - obstacle_pose[1][1]);

    /* 队形信息设定 */
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        circle(i, 0) = characteristic_1*cos(i*2*pi/swarm_robot_id.size());
        circle(i, 1) = characteristic_1*sin(i*2*pi/swarm_robot_id.size());
    }
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        star(i, 0) = characteristic_2*cos((i - 1)*pi/2)*int((i + 3)/4);
        star(i, 1) = characteristic_2*sin((i - 1)*pi/2)*int((i + 3)/4);
    }
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        wedge(i, 0) = characteristic_2*cos(-2*pi/3)*int((i + 1)/2);
        wedge(i, 1) = characteristic_2*sin(-2*pi/3)*pow(-1, i)*int((i + 1)/2);
    }
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        colume1(i, 0) = characteristic_2*i;
        colume1(i, 1) = 0;
    }
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        colume2(i, 0) = characteristic_3*int(i/2);
        colume2(i, 1) = characteristic_3*pow(-1, int(i/2));
    }
    for (int i = 0; i < swarm_robot_id.size(); ++i){
        colume3(i, 0) = characteristic_2*int(i/3);
        colume3(i, 1) = characteristic_4*(i%3 - 1);
    }
    ...
    for (int i = 0; i < role_assignment; ++i){
        for (int j = 0; j < swarm_robot_id.size(); ++j){
            ...
            FormationA_x(j, i + role_assignment) = star(array_flag[j], 0) - star(array_flag[0], 0);
            FormationA_y(j, i + role_assignment) = star(array_flag[j], 1) - star(array_flag[0], 1);
            FormationA_x(j, i + 2*role_assignment) = wedge(array_flag[j], 0) - wedge(array_flag[0], 0);
            FormationA_y(j, i + 2*role_assignment) = wedge(array_flag[j], 1) - wedge(array_flag[0], 1);
            FormationB_x(j, i) = colume1(array_flag[j], 0) - colume1(array_flag[0], 0);
            FormationB_y(j, i) = colume1(array_flag[j], 1) - colume1(array_flag[0], 1);
            FormationB_x(j, i + role_assignment) = colume2(array_flag[j], 0) - colume2(array_flag[0], 0);
            FormationB_y(j, i + role_assignment) = colume2(array_flag[j], 1) - colume2(array_flag[0], 1);
            FormationB_x(j, i + 2*role_assignment) = colume3(array_flag[j], 0) - colume3(array_flag[0], 0);
            FormationB_y(j, i + 2*role_assignment) = colume3(array_flag[j], 1) - colume3(array_flag[0], 1);
        }
    ...
    while(!angle_conv){
        ...
        for(int i = 0; i < swarm_robot_id.size(); i++) {
            yaw_control(swarm_robot, i, del_theta(i));
        }
    }
    ...
    while(! pos_conv) {
        ...
        formation_selection(formationA_num, zeta_A);
        cal_formation(formationA_num, FormationA_x, FormationA_y);
        double error_x = obstacle_centor[0] - Formation_centor[0] - 2*characteristic_1;
